...
module nerv #(
	...
	//identify the ifid-- nnpc
	logic ifid_rd_enable;
	logic ifid_rd_enable_q; 
	...
	assign insn_p = stall ? 32'h0000_0013 : imem_data;
	...
	always_comb begin
		...
		if(insn_opcode == OPCODE_BRANCH && insn_p[6:0] == 7'b1110011) begin 
			kill = 1; 
		end 
		else begin 
			kill = 0;
		end
		if((insn_opcode == OPCODE_JAL || insn_opcode == OPCODE_JALR) && pc != 0 && pc[31] == 0) begin 
			stall = 1; 
		end 
		else begin 
			stall = 0;
        ...
        if (insn_p[6:0] == OPCODE_LOAD) mem_rd_enable = 1; 
		...
		case (insn_opcode)
			...
			OPCODE_BRANCH: begin
				case (insn_funct3)
					3'b 000 /* BEQ  */: begin if (rs1_value == rs2_value) npc = nnpc + imm_b_sext; end
					3'b 001 /* BNE  */: begin if (rs1_value != rs2_value) npc = nnpc + imm_b_sext; end
					3'b 100 /* BLT  */: begin if ($signed(rs1_value) < $signed(rs2_value)) npc = nnpc + imm_b_sext; end
					3'b 101 /* BGE  */: begin if ($signed(rs1_value) >= $signed(rs2_value)) npc = nnpc + imm_b_sext; end
					3'b 110 /* BLTU */: begin if (rs1_value < rs2_value) npc = nnpc + imm_b_sext; end
					3'b 111 /* BGEU */: begin if (rs1_value >= rs2_value) npc = nnpc + imm_b_sext; end
					default: illinsn = 1;
				endcase
				if (npc & 32'b 11) begin
					illinsn = 1;
	...
	always @(posedge clock) begin
		...
		insn <= kill ? 32'h0000_0013 : mem_rd_enable_q ? insn : insn_p; 
